...
public class DatabaseSeeder {
    ...
    public void init() {
        ...
        var joao = User.builder().email("joao_23cm@fiap.com.br").password(password).build();
        var maria = User.builder().email("maria_dondoca@fiap.com.br").password("12345").build();

        userRepository.saveAll(List.of(joao, maria));

        var categories = List.of(
                Category.builder().name("Educação").user(joao).icon("Book").build(),
                Category.builder().name("Lazer").user(joao).icon("Dices").build(),
                Category.builder().name("Transporte").user(maria).icon("Bus").build(),
                Category.builder().name("Moradia").user(maria).icon("House").build(),
                Category.builder().name("Saúde").user(maria).icon("Heart").build());
